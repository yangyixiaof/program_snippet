...
public class IRGeneratorForOneMethod extends ASTVisitor {
	...
	public boolean visit(WhileStatement node) {
		...
		post_visit_task.put(node.getExpression(), new Runnable() {
			@Override
			public void run() {
				IRGeneratorHelper.GenerateGeneralIR(node, node.getExpression(), irfom, temp_statement_set, IRMeta.While);
				StatementOverHandle();
			}
		});
		return super.visit(node);
	...
	public boolean visit(EnhancedForStatement node) {
		...
		post_visit_task.put(node.getExpression(), new Runnable() {
			@Override
			public void run() {
				IRGeneratorHelper.GenerateGeneralIR(node, node.getExpression(), irfom, temp_statement_set, IRMeta.EnhancedFor);
				StatementOverHandle();
			}
		});
		return super.visit(node);
	...
	public void endVisit(ContinueStatement node) {
		...
		if (n != null && loop_bind.containsKey(n))
		{
			IRGeneratorHelper.GenerateNoVariableBindingIR(node, node.getLabel(), irfom, loop_bind.get(n), IRMeta.Continue);
		}
	}
	
	@Override
	public boolean visit(VariableDeclarationStatement node) {
		// no need to do anything, all things are in VariableDeclarationFragment.
		return super.visit(node);
	}
	...
	public boolean visit(VariableDeclarationFragment node) {
		post_visit_task.put(node.getName(), new Runnable() {
			@Override
			public void run() {
				IRGeneratorHelper.GenerateGeneralIR(node, node.getName(), irfom, temp_statement_set, IRMeta.VariabledDeclare);
				StatementOverHandle();
			}
		});
		return super.visit(node);
	...
	public boolean visit(Assignment node) {
		post_visit_task.put(node.getLeftHandSide(), new Runnable() {
			@Override
			public void run() {
				IRGeneratorHelper.GenerateGeneralIR(node, node.getLeftHandSide(), irfom, temp_statement_set, IRMeta.LeftHandAssign);
			}
		});
		post_visit_task.put(node.getRightHandSide(), new Runnable() {
			@Override
			public void run() {
				IRGeneratorHelper.GenerateGeneralIR(node, node.getRightHandSide(), irfom, temp_statement_set, IRMeta.RightHandAssign);
			}
		});
		return super.visit(node);
	}
	...
	public boolean visit(SynchronizedStatement node) {
		post_visit_task.put(node.getExpression(), new Runnable() {
			@Override
			public void run() {
				IRGeneratorHelper.GenerateGeneralIR(node, node.getExpression(), irfom, temp_statement_set, IRMeta.Synchronized);
				StatementOverHandle();
			}
		});
		return super.visit(node);
	...
	public boolean visit(SwitchStatement node) {
		...
		post_visit_task.put(node.getExpression(), new Runnable() {
			@Override
			public void run() {
				IRGeneratorHelper.GenerateGeneralIR(node, node.getExpression(), irfom, temp_statement_set, IRMeta.Switch);
			}
		});
		return super.visit(node);
	}
	...
	public boolean visit(SimpleName node) {
		...
		{
			// handle loop_bind, just for no variable bind statements such as break and continue.
